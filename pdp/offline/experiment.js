...
function expStart(subject, session) {
    ...
    jsPsych.init({
        ...
        on_close: function() {
            jsPsych.data.get().ignore("correct").localSave("csv", `PDP_Subject_${subjectID}_Session_${sessionNumber}_quitted_output.csv`);
        },
        ...
        on_finish: function () {
            jsPsych.data.get().ignore("correct").localSave("csv", `PDP_Subject_${subjectID}_Session_${sessionNumber}_output.csv`);
        }
    })
}